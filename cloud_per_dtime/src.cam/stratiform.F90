...
!++ added by Urs Beyerle
    call addfld ('TGCLDTWPL', 'gram/m2 ', 1   , 'A', 'Total large scale grid box avg cloud total water path'   ,phys_decomp)
    call addfld ('TGCLDIWPL', 'gram/m2 ', 1   , 'A', 'Total large scale grid box avg cloud ice water path'     ,phys_decomp)
    call addfld ('TGCLDLWPL', 'gram/m2 ', 1   , 'A', 'Total large scale grid box avg cloud liquid water path'  ,phys_decomp)
!--
...
! Initialize variables
    gicewp(:ncol,:pver) = 0._r8
    gliqwp(:ncol,:pver) = 0._r8	
    tgicewp(:ncol) = 0._r8
    tgliqwp(:ncol) = 0._r8
    tgwp(:ncol) = 0._r8
    ...
    do k = 1, pver
       do i = 1,ncol
          gicewp(i,k) = state1%q(i,k,ixcldice)*state1%pdel(i,k)/gravit*1000.0_r8  ! Grid box ice water path.
          gliqwp(i,k) = state1%q(i,k,ixcldliq)*state1%pdel(i,k)/gravit*1000.0_r8  ! Grid box liquid water path.
...
!          cliqwp(i,k) = gliqwp(i,k) / max(0.01_r8,cldn(i,k))                 ! In-cloud liquid water path.
   ...
   call outfld( 'TGCLDTWPL' , tgwp,       pcols, lchnk )
   call outfld( 'TGCLDIWPL' , tgicewp,    pcols, lchnk )
   call outfld( 'TGCLDLWPL' , tgliqwp,    pcols, lchnk )
!--
